...
#
# Vendored from https://tcp.hy2.sh/ (upstream hash: cd7615dd64836d8b239124cad776ac9e5a330830147e1a6892cb69e1ae6c9de6)
# This copy is intentionally local so the TUI does not depend on the upstream
# installer URL at execution time. Package-manager branches below are local
# portability additions; DKMS/module behavior remains the upstream flow.
...
detect_package_manager() {
  ...
  if has_command xbps-install; then
    PACKAGE_MANAGEMENT_INSTALL='xbps-install -y'
    return 0
  fi

  if has_command emerge; then
    PACKAGE_MANAGEMENT_INSTALL='emerge --ask=n'
    return 0
  fi
...
install_linux_headers() {
  ...
  if has_command pacman; then
    ...
    install_software "linux-headers"
  elif has_command zypper; then
    install_software "kernel-default-devel"
  elif has_command xbps-install; then
    install_software "linux-headers"
  elif has_command emerge; then
    install_software "sys-kernel/linux-headers"
  else
    # unsupported
